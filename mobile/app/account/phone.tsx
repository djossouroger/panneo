...
import { ApiError, changePhone, friendlyError, requestPhoneChange } from '../../lib/api';
...
export default function ChangePhoneScreen() {
  ...
  const sendCode = async () => {
    ...
    try {
      ...
      await requestPhoneChange(session.token, phone.trim());
      setCodeSent(true);
    } catch (err) {
  ...
  const confirm = async () => {
    ...
    try {
      ...
      const updated = await changePhone(session.token, phone.trim(), code.trim());
      await saveUser(updated);
      Alert.alert('Numéro mis à jour', 'Votre nouveau numéro de téléphone est actif.', [{ text: 'OK', onPress: () => router.replace('/account') }]);
    } catch (err) {
      if (await handleSessionExpired(err)) {
        router.replace('/login');
        return;
  ...
  return (
    <ScreenContainer>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Changer mon téléphone</Text>
      </View>

      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <ErrorMessage message={error} />
        <Text style={styles.hint}>
          Un code de vérification sera envoyé à votre nouveau numéro. Saisissez-le pour confirmer le changement.
        </Text>

        <AppInput label="Nouveau numéro" value={phone} onChangeText={setPhone} placeholder="+229 90 00 00 00" keyboardType="phone-pad" icon={<Smartphone size={18} color={colors.muted} />} />
